...
def fund_account_from_ganache(w3, ganache_private_key, target_address, amount_eth=1.0):
    try:
        ...
        if source_balance_eth < amount_eth + 0.001:
...
def record_model_parameters(model_params, training_timestamp):
    ...
    contract ModelStorage {
        ...
        event ModelStored(string timestamp);

        function store(string memory _modelData) public {
            modelData = _modelData;
            emit ModelStored(_modelData);
        }
    ...
    if balance_eth < 0.01:
        ...
        if not funded:
            raise ValueError("Could not fund account automatically.")
    ...
    ModelStorage = w3.eth.contract(abi=abi, bytecode=bytecode)
    nonce = w3.eth.get_transaction_count(sender_address)

    try:
        print("Deploying model storage contract...")
        deploy_tx = ModelStorage.constructor().build_transaction({
            'from': sender_address,
            'nonce': nonce,
            'gas': 2000000,
            'gasPrice': min_gas_price
        ...
        contract_address = tx_receipt.contractAddress
        print(f"Model contract deployed successfully at: {contract_address}")
        print(f"Gas used for deployment: {tx_receipt.gasUsed}")
    ...
    blockchain_data = {
        ...
        "model_type": "ANN_Classifier"
    }
    json_data = json.dumps(blockchain_data)

    nonce += 1
    try:
        print("Storing model parameters on blockchain...")
        store_tx = contract.functions.store(json_data).build_transaction({
            'from': sender_address,
            'nonce': nonce,
            'gas': 1000000,
        ...
        print(f"Model parameters stored successfully! Transaction hash: {store_tx_hash.hex()}")
        print(f"Gas used for storage: {store_receipt.gasUsed}")
        ...
        if "insufficient funds" in str(e).lower():
            ...
            raise Exception(f"Model storage failed: {e}")

    return {
        "message": f"Model parameters stored successfully on blockchain at {contract_address}",
        "contract_address": contract_address,
        "transaction_hash": store_tx_hash.hex(),
        "abi": abi,
        "bytecode": bytecode
    }